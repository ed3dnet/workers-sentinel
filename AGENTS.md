...
**Runner history note**: the worker test toolchain was migrated from `@cloudflare/vitest-pool-workers@0.12.4` (vitest 3, workerd 1.20260114.0) to `@cloudflare/vitest-plugin@1.2.1` (vitest 4, current workerd). The old runner exhibited the open upstream bug [workers-sdk#15446](https://github.com/cloudflare/workers-sdk/issues/15446) — `SELF.fetch` latency grows cumulatively within one worker (6.5ms → 368ms over ~1500 requests there; here even `/api/health` climbed until a marginal test timed out; a fresh workerd resets it). Exercising the passkey credential-delete flow was the local tripwire, which forced those tests into a quarantined worker; the migration fixed it and the quarantine was removed. Keep the plugin reasonably current if similar runner anomalies appear. (Related: the `Can't read from request stream after response has been sent` exceptions from the old runs are the known respond-before-body-consumption error, workerd#918/#1730 — the routes now drain or cancel bodies before every early response.)
